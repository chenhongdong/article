...
function render(data) {
    ...
    for (let i = 0; i < l.length; i++) {
        ...
        const { lb, loop, uid } = l_xmlattr
        
        let dStr = ""; //地铁线路点
        let isLb = false; //是否圆润拐点
        for (let j = 0; j < p.length; j++) {
            const { x, y, lb, st, ex, rc } = p[j].p_xmlattr
            if (isLb) {
                isLb = false
                dStr += x + " " + y + " "
            } else {
        ...
        const { lb, lc, lbx, lby } = l_xmlattr
        let path = $.svg('path').appendTo('#g-box')
        path.attr({
            d: $.trim(dStr),
            lb: lb
        }).css("stroke", "#" + lc.split("x")[1]);
        let text = $.svg('text').appendTo('#g-box').html(lb).addSvgClass("subway-name")
        text.attr({
            x: lbx - 10,
            y: lby + 15,
            ...
            if (st) {
                if (ex) {
                    if (!repeatStr.includes(uid)) {
                        let image = $.svg('image').appendTo('#g-box')
                        image.attr({
                            width: "20",
                            height: "20",
                            x: x - 10,
                    ...
                    let circle = $.svg('circle').appendTo('#g-box')
                    circle.attr({
                        cx: x,
                        cy: y,
                        r: 4
                ...
                if (!repeatStr.includes(uid)) {
                    let text = $.svg('text').appendTo('#g-box').html(lb).addSvgClass("station-name")
                    text.attr({
                        x: x + rx + 2,
                        y: y + ry + 12,
                    });
